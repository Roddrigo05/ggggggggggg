...
import jogo.Java.FastNoiseLite;
...
public class VoxelWorld {
    ...
    private final HashMap<Byte, Integer> blockCount = new HashMap<>();
    ...
    public void setBlock(int x, int y, int z, byte id) {
        ...
        if(!blockCount.containsKey(id)) {
            blockCount.put(id, 1);
        } else {
            blockCount.put(id, blockCount.get(id) + 1);
        }
    }
    ...
    public boolean breakAt(int x, int y, int z, jogo.gameobject.character.Player player) {
        if (!inBounds(x, y, z)) return false;

        byte voxelID = getBlock(x, y, z);
        if (voxelID == VoxelPalette.BEDROCK_ID)
            return false;
        else {
            if (player != null && player.getinventario() != null) {
                player.getinventario().adicionarItem(voxelID, 1);
    ...
    public boolean colocarBloco(int x, int y, int z, byte id) {
    ...
    public void generateLayers() {
        ...
        for (int x = 0; x < sizeX; x++) {
            for (int z = 0; z < sizeZ; z++) {
                ...
                float noiseValue = baseNoise.GetNoise(x, z);
                float detailNoiseValue = detailNoise.GetNoise(x, z);

                //soma os dois ruidos e divide por pois pois o fastnoise final tem de variar entre -1 e 1
                float value = (noiseValue + detailNoiseValue)/2f;
                // impede que o numero ultrapasse o 1 e multiplica pela altura da montanha para criar motanhas maiores
                // sem ultrapassar o maximo ou seja o height  e o valor maximo da primeira camada em cada ponto
                int height = (int)(((value + 1f) / 2f) * mountainHeight);

                //organiza como os blocos vai ser  organizados
                for (int y = 0; y <= height; y++) {
                    ...
                    else if (getBlock(x, y+1, z) == VoxelPalette.DIRT_ID) {
                    ...
                    if (y < caveHeight && y > 1) {
                        float cave = caveNoise.GetNoise(x, y, z);
                        if (cave < -0.2f && getBlock(x, y, z) != VoxelPalette.BEDROCK_ID) {
                            setBlock(x, y, z, VoxelPalette.AIR_ID);
                        }
                    }
                }

                //subsitui alguns blocos de pedra por minerios
                for (int y = 0; y < sizeY; y++) {
                    if (getBlock(x, y, z) == VoxelPalette.STONE_ID) {

                        if (y <= 4 && random.nextFloat() < 0.003f)
                            setBlock(x, y, z, VoxelPalette.DIAMOND_ID);

                        if (y <= 3 && random.nextFloat() < 0.003f)
                            setBlock(x, y, z, VoxelPalette.EMERALD_ID);

                        if (y >= 3 && y < 16 && random.nextFloat() < 0.02f)
                            setBlock(x, y, z, VoxelPalette.IRON_ID);

                        if (y > 3 && y <= 12 && random.nextFloat() < 0.012f)
                            setBlock(x, y, z, VoxelPalette.GOLD_ID);

                        if (y > 4 && y <= 8 && random.nextFloat() < 0.01f)
                            setBlock(x, y, z, VoxelPalette.LAPIS_ID);

                        if (y > 3 && y <= 10 && random.nextFloat() < 0.015f)
                            setBlock(x, y, z, VoxelPalette.REDSTONE_ID);
                    }
                }

                //gerador de madeira que varia entre 3 a 6 blocos por arvore
                if (getBlock(x, height, z) == VoxelPalette.GRASS_ID && random.nextFloat() < 0.001f && height<sizeY-8) {
                    int altura = random.nextInt(3, 6);

                    for (int h = 1; h <= altura; h++) {
                        setBlock(x, height + h, z, VoxelPalette.WOOD_ID);
                    }


                    int topo = height + altura;
                    for (int dx = -2; dx <= 2; dx++) {
                        for (int dz = -2; dz <= 2; dz++) {
                            for (int dy = 0; dy <= 2; dy++) {
                                ...
                                //impede que todos os blovos spawnem fora dos limites da engine
                                if (fx < 0 || fy < 0 || fz < 0 || fx >= sizeX || fy >= sizeY || fz >= sizeZ)
                                    continue;

                                //torna os valores negativos em positivos e cooloca os blocos de folhas
                                int dist = Math.abs(dx) + Math.abs(dz) + dy;
                                if (dist <= 3 && getBlock(fx, fy, fz) == VoxelPalette.AIR_ID) {
