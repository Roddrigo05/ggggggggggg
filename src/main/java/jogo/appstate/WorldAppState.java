...
public class WorldAppState extends BaseAppState {
    ...
    protected void initialize(Application app) {
        ...
        voxelWorld = new VoxelWorld(assetManager, 320, 45, 320);
    ...
    public void update(float tpf) {
        if (input != null && input.isMouseCaptured() && input.consumeBreakRequested()) {
            var hit = voxelWorld.pickFirstSolid(cam, 6f);
            hit.ifPresent(cell -> {
                var player = playerAppState != null ? playerAppState.getPlayer() : null;


                if (voxelWorld.breakAt(cell.x, cell.y, cell.z, player)) {
